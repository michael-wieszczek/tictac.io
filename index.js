...
io.on("connection", function(socket) {
    ...
    socket.on("filledCounter", (data) => {
        socket.emit("filledCounter.set", data);
        opponentOf(socket).emit("filledCounter.set", data);
    });
...
function join(socket) {
    // Edit these later to provide 4? player functionality
    players[socket.id] = {
        opponent: unmatched,
        symbol: "X",
        socket: socket
